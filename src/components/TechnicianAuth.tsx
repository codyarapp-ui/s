...
import { User, Lock, Phone, MapPin, Check, Briefcase, FileText, UploadCloud, AlertCircle, Eye, EyeOff, Sparkles, Clock, X, Plus, Search, ChevronDown, RefreshCw } from 'lucide-react';
...
export const TechnicianAuth: React.FC<TechnicianAuthProps> = ({
  ...
  // Real-time verification polling & status check
  const [isCheckingStatus, setIsCheckingStatus] = useState<boolean>(false);
  const [lastCheckTime, setLastCheckTime] = useState<string>('');

  const checkVerificationStatus = React.useCallback(async (isManual = false) => {
    if (!loggedInTech || loggedInTech.isVerified) return;
    if (isManual) setIsCheckingStatus(true);
    try {
      const techId = loggedInTech.id || '';
      const techPhone = loggedInTech.phone || '';
      const res = await fetch(`/api/technicians/${encodeURIComponent(techId)}/status?phone=${encodeURIComponent(techPhone)}&t=${Date.now()}`);
      if (res.ok) {
        const data = await res.json();
        if (data && (data.isVerified === true || data.is_verified === 1)) {
          const verifiedTech = {
            ...loggedInTech,
            isVerified: true,
            status: data.techStatus || data.status || 'active',
            documents: loggedInTech.documents && loggedInTech.documents.length > 0 ? loggedInTech.documents : [{ name: 'تایید صلاحیت صنف و مدیریت', url: '' }]
          };
          onRegister(verifiedTech);
          window.dispatchEvent(new CustomEvent('technician_verified', { detail: { id: verifiedTech.id, isVerified: true } }));
          triggerNotification('🎉 مدارک شما تایید شد!', 'مدیریت سامانه کدیار صلاحیت و مدارک شما را تایید نمود. پنل تخصصی شما فعال شد.', 'success');
          return;
        }
      }
      setLastCheckTime(new Date().toLocaleTimeString('fa-IR'));
      if (isManual) {
        triggerNotification('در انتظار بررسی', 'پرونده شما در صف ممیزی مدیریت قرار دارد. به محض تایید توسط مدیر، دسترسی شما خودکار باز خواهد شد.', 'info');
      }
    } catch (e) {
      console.warn('Status check poll failed:', e);
    } finally {
      if (isManual) setIsCheckingStatus(false);
    }
  }, [loggedInTech, onRegister, triggerNotification]);
  ...
  React.useEffect(() => {
    ...
    // Poll every 3 seconds for instant synchronization
    const interval = setInterval(() => {
      checkVerificationStatus(false);
    }, 3000);

    const handleGlobalVerified = (e: any) => {
      const detail = e.detail;
      if (detail && (String(detail.id) === String(loggedInTech.id) || String(detail.id) === String(loggedInTech.phone))) {
        if (detail.isVerified) {
          checkVerificationStatus(false);
        }
      }
    };
    window.addEventListener('technician_verified', handleGlobalVerified);

    return () => {
      clearInterval(interval);
      window.removeEventListener('technician_verified', handleGlobalVerified);
    };
  }, [loggedInTech?.id, loggedInTech?.isVerified, checkVerificationStatus]);
  ...
  const handleTechForgotRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotStep(2);
      triggerNotification('کد تایید پیامکی ارسال شد', 'کد تایید بازیابی کلمه عبور با پیامک به شماره همراه شما ارسال گردید.', 'sms');
    } catch (err) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Strict server truth check for suspension and verification
      const isServerSuspended = Boolean(
        data.technician?.status === 'suspended' ||
        data.technician?.status === 'blocked' ||
        loggedUser.status === 'suspended' ||
        loggedUser.status === 'blocked'
      );

      const serverVerified = !isServerSuspended && Boolean(
        data.technician?.isVerified === true ||
        data.technician?.is_verified === 1 ||
        loggedUser.isVerified === true ||
        loggedUser.is_verified === 1
      );
      ...
      if (!foundTech) {
        ...
        foundTech = {
          id: data.technician?.id || `tech_${loggedUser.id}`,
          name: data.technician?.name || data.technician?.fullName || loggedUser.full_name || 'تکنسین گرامی',
          phone: inputPhoneEng,
          password: loginPassword,
          specialty: data.technician?.specialty || (regSpecialties.length > 0 ? regSpecialties : ['تعمیرات عمومی']),
          rating: 5.0,
          completedOrders: 0,
          balance: 0,
          isVerified: serverVerified,
          status: isServerSuspended ? 'suspended' : (serverVerified ? 'active' : 'pending'),
          activeLocation: data.technician?.activeLocation || loggedUser.city || 'تهران',
          documents: data.technician?.documents || [],
          avatarUrl: data.technician?.avatarUrl || ""
        };
        onRegister(foundTech);
      } else {
        // Server is absolute source of truth: synchronize isVerified and status
        foundTech = { 
          ...foundTech, 
          isVerified: serverVerified,
          status: isServerSuspended ? 'suspended' : (serverVerified ? 'active' : (foundTech.status || 'pending')),
          documents: serverVerified 
            ? (foundTech.documents && foundTech.documents.length > 0 ? foundTech.documents : [{ name: 'تایید صلاحیت صنف', url: '' }])
            : (isServerSuspended ? [] : foundTech.documents)
        };
        onRegister(foundTech);
      }

      // Display proper state notification
      if (isServerSuspended || !foundTech.isVerified) {
        triggerNotification(
          '⛔ حساب همکاری معلق است',
          `جناب ${foundTech.name}، حساب کاربری شما توسط مدیریت کدیار۲۴ معلق شده است و دسترسی به سفارش‌های کار مسدود می‌باشد.`,
          'error'
        );
      } else {
      ...
      localStorage.setItem('ir_logged_in_tech_id', foundTech.id);
      localStorage.setItem('ir_current_role', 'technician');
      localStorage.setItem('session_user_role', 'technician');
      window.dispatchEvent(new CustomEvent('user_login_completed', { detail: { role: 'technician', id: foundTech.id } }));
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    const trimmedCity = regCity.trim();

    if (!trimmedName || !regPhone || !trimmedPass || !trimmedCity) {
      const msg = 'تکمیل تمامی مشخصات فردی ستاره‌دار (نام و نام خانوادگی، شماره همراه، شهر و رمز عبور) الزامی است.';
      setRegisterError(msg);
      triggerNotification('تکمیل مشخصات فردی', msg, 'warning');
      return;
    }

    if (trimmedName.length < 3) {
      const msg = 'نام و نام خانوادگی باید حداقل ۳ حرف باشد.';
      setRegisterError(msg);
      triggerNotification('نام نامعتبر', msg, 'warning');
      return;
    }
    ...
    if (!checkPhone.isValid) {
      ...
      triggerNotification('شماره همراه نامعتبر', checkPhone.error || 'قالب شماره همراه صحیح نیست.', 'warning');
      return;
    }

    if (trimmedPass.length < 4) {
      const msg = 'کلمه عبور اختصاصی حداقل باید ۴ کاراکتر باشد.';
      setRegisterError(msg);
      triggerNotification('کلمه عبور کوتاه', msg, 'warning');
      return;
    }

    if (regSpecialties.length === 0) {
      const msg = 'لطفاً حداقل یک مهارت یا تخصص فنی از لیست انتخاب نمایید.';
      setRegisterError(msg);
      triggerNotification('انتخاب تخصص فنی', msg, 'warning');
      return;
    }
    ...
    if (uploadedDocName.trim() || selectedFileObj) {
      const cleanTitle = (uploadedDocName.trim() || selectedFileObj?.name || 'مدرک هویتی و مهارتی').toLowerCase();
    ...
    const docImagesList: string[] = [];
    finalDocs.forEach(d => {
      try {
        if (d.startsWith('{')) {
          const parsed = JSON.parse(d);
          const url = parsed.fileData || parsed.fileUrl || parsed.url;
          if (url) docImagesList.push(url);
        } else if (d.startsWith('http') || d.startsWith('/uploads/') || d.startsWith('data:image')) {
          docImagesList.push(d);
        }
      } catch (e) {}
    });

    if (finalDocs.length === 0 && docImagesList.length === 0) {
      const msg = 'بارگذاری و ارسال حداقل یک مدرک معتبر هویتی یا صلاحیت فنی (کارت ملی، مدرک فنی‌وحرفه‌ای یا پروانه کسب) الزامی است.';
      setRegisterError(msg);
      triggerNotification('بارگذاری مدرک الزامی است', msg, 'warning');
  ...
  const handleSelectQuickDocTitle = (docName: string, isProfileUpdate = false) => {
    if (isProfileUpdate && loggedInTech) {
      const currentDocs = loggedInTech.documents || [];
      const already = currentDocs.some(d => {
        let dName = d;
        try { if (d.startsWith('{')) dName = JSON.parse(d).name || ''; } catch(e){}
        return dName.trim().toLowerCase().includes(docName.trim().toLowerCase()) || docName.trim().toLowerCase().includes(dName.trim().toLowerCase());
      });
      if (already) {
        triggerNotification('مدرک ثبت‌شده', `مدرک «${docName}» قبلاً در پرونده شما ثبت شده و بر اساس مقررات کدیار۲۴ قابل ویرایش، تغییر یا حذف نمی‌باشد.`, 'warning');
        return;
      }
    }
  ...
  if (loggedInTech && !loggedInTech.isVerified) {
    ...
    return (
      <div className="bg-slate-50 rounded-2xl border border-slate-200/90 shadow-xl p-4 sm:p-8 max-w-xl mx-auto my-6 font-sans">
                ...
                onClick={() => {
                  ...
                  if (onResubmitDocs) {
                    ...
                    const proceedResubmit = (fileDataUrl: string, fileType: string) => {
                      const currentDocs = loggedInTech.documents || [];
                      const targetName = uploadedDocName.trim().toLowerCase();
                      const alreadyExists = currentDocs.some(d => {
                        let dName = d;
                        try {
                          if (d.startsWith('{')) dName = JSON.parse(d).name || '';
                        } catch(e) {}
                      ...
                      if (alreadyExists) {
                        setIsUploading(false);
                        triggerNotification('عدم امکان تغییر مدرک', `مدرک «${uploadedDocName}» قبلاً آپلود شده است. طبق قوانین، مدارک ثبت‌شده به عنوان اسناد غیرقابل تغییر بایگانی می‌شوند و امکان ویرایش یا جایگزینی آن وجود ندارد.`, 'error');
                        return;
                      }
        ...
        {/* Real-time sync and manual verification check */}
        <div className="mt-5 p-4 rounded-2xl bg-white border border-slate-200/90 shadow-sm space-y-3 text-right">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-black text-slate-700">اتصال برخط و زنده به سرور مرکزی</span>
            </div>
            {lastCheckTime && (
              <span className="text-[9.5px] font-bold text-slate-400">
                آخرین استعلام: {lastCheckTime}
              </span>
            )}
          </div>
          <p className="text-[11px] leading-relaxed text-slate-500 font-medium">
            سامانه هر ۳ ثانیه وضعیت تایید شما را از سرور استعلام می‌کند. به محض کلیک مدیر روی دکمه تایید، دسترسی به پنل تکنسین بدون نیاز به خروج به طور خودکار باز می‌شود.
          </p>
          <button
            type="button"
            disabled={isCheckingStatus}
            onClick={() => checkVerificationStatus(true)}
            className="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-xl py-2.5 px-4 text-xs font-black transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm disabled:opacity-50"
          >
            <RefreshCw className={`w-4 h-4 ${isCheckingStatus ? 'animate-spin' : ''}`} />
            <span>{isCheckingStatus ? 'در حال استعلام از سرور...' : '🔄 استعلام دستی و بروزرسانی وضعیت تایید پرونده'}</span>
          </button>
        </div>
            ...
            <div className="text-center pt-3 border-t border-slate-200 space-y-2">
              ...
              {onBackToHome && (
                <button
                  ...
                  className="text-xs text-slate-500 hover:text-slate-800 font-bold hover:underline cursor-pointer block mx-auto"
                >
                  ← بازگشت به صفحه اصلی سامانه کدیار۲۴
                </button>
              )}
            </div>
          </form>
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-5">
          {registerSuccess ? (
            ...
            <>
              ...
              <div className="text-center pt-2.5 border-t border-slate-200 space-y-2">
                <p className="text-xs text-slate-500">
