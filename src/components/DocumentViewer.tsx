...
interface DocumentViewerProps {
  techName: string;
  docName: any; // can be a legacy name, URL, or object / JSON-serialized string of { name, fileData, fileType }
  onClose: () => void;
}
...
export const DocumentViewer: React.FC<DocumentViewerProps> = ({ techName, docName, onClose }) => {
  ...
  // Parse JSON-serialized real document or direct URL if applicable
  const parsedDoc = React.useMemo(() => {
    try {
      if (docName) {
        if (typeof docName === 'object') {
          const dataUrl = docName.fileData || docName.fileUrl || docName.url || docName.data || docName.src || '';
          const name = docName.name || docName.title || docName.fileName || 'مدرک بارگذاری‌شده';
          const fileType = docName.fileType || (dataUrl && dataUrl.includes('.pdf') ? 'application/pdf' : 'image/jpeg');
          return {
            isReal: Boolean(dataUrl),
            name,
            fileData: dataUrl,
            fileType
          };
        ...
        if (typeof docName === 'string') {
          ...
          if (trimmed.startsWith('http://') || trimmed.startsWith('https://') || trimmed.startsWith('/uploads/') || trimmed.startsWith('data:')) {
            return {
              isReal: true,
              name: 'تصویر واقعی مدرک بارگذاری‌شده',
              fileData: trimmed,
          ...
          if (trimmed.startsWith('{')) {
            ...
            const dataUrl = parsed.fileData || parsed.fileUrl || parsed.url || parsed.data || parsed.src;
            if (dataUrl) {
              return {
                isReal: true,
                name: parsed.name || parsed.title || 'سند بارگذاری‌شده',
                fileData: dataUrl,
    ...
    return {
      isReal: false,
      name: typeof docName === 'string' ? docName : (docName?.name || 'مدرک فنی'),
      fileData: '',
      fileType: ''
    };
  }, [docName]);
