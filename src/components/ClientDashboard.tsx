...
export const ClientDashboard: React.FC<ClientDashboardProps> = ({
  ...
  // Fetch Wallet, Referrals, and Support Tickets on client load with real-time polling
  useEffect(() => {
    if (currentUser) {
      ...
      // 2. Fetch support tickets list with silent real-time polling
      const fetchUserTickets = (isSilent = false) => {
      ...
      // Real-time polling every 3 seconds for immediate chat updates without page reload
      const ticketPollingInterval = setInterval(() => {
  ...
  const handleSendTicketReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (updatedTk) {
          setTicketsList(prev => prev.map(t => String(t.id) === String(activeTicketId) ? updatedTk : t));
        }
        setTicketReplyInput('');
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedName || !regPhone || !trimmedPass || !trimmedCity) {
      triggerNotification('تکمیل فیلدها', 'پر کردن تمامی اطلاعات ستاره‌دار فردی (نام و نام خانوادگی، شماره همراه، شهر و رمز عبور) الزامی است.', 'warning');
      return;
    }

    if (trimmedName.length < 3) {
      triggerNotification('نام نامعتبر', 'نام و نام خانوادگی باید حداقل ۳ حرف باشد.', 'warning');
      return;
    }

    if (trimmedPass.length < 4) {
      triggerNotification('کلمه عبور کوتاه', 'کلمه عبور حداقل باید ۴ کاراکتر باشد.', 'warning');
      return;
    }
  ...
  const handleEditProfileSubmit = async (e: React.FormEvent) => {
    ...
    setEditLoading(true);
    try {
      const token = localStorage.getItem('session_user_id') || currentUser?.id || '';
      ...
      <div className="bg-slate-900 text-white rounded-3xl p-6 relative overflow-hidden border border-slate-800 shadow-md">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4 text-right">
            <div className="w-14 h-14 bg-gradient-to-tr from-blue-700 to-emerald-500 rounded-2xl flex items-center justify-center text-white font-black text-lg shadow-lg overflow-hidden shrink-0">
              {currentUser.avatar_url || currentUser.avatarUrl ? (
                <img 
                  src={currentUser.avatar_url || currentUser.avatarUrl} 
                  alt={currentUser.full_name || 'کاربر'} 
              ...
              {ticketsLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="border border-slate-150 rounded-2xl p-3 bg-slate-50 flex flex-col justify-between min-h-[220px]">
                    {(() => {
                      const activeTicket = ticketsList.find(t => String(t.id) === String(activeTicketId));
                      if (!activeTicket) {
                      ...
                      return (
                        <div className="flex flex-col h-full justify-between gap-3 text-right">
                          <div className="border-b border-slate-200/60 pb-1.5">
                            ...
                            <span className="text-[9px] text-slate-400">آخرین بروزرسانی: {(() => {
                              try {
                                const d = new Date(activeTicket.updatedAt || activeTicket.updated_at || activeTicket.createdAt || activeTicket.created_at);
                          ...
                          <div className="flex-1 space-y-2.5 overflow-y-auto max-h-44 text-[10.5px] pr-1">
                            ...
                            {activeTicket.replies && activeTicket.replies.map((rp: any, rIdx: number) => (
                              <div
                                key={rp.id || `reply_${rIdx}_${rp.createdAt || ''}`}
