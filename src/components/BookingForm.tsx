...
export const BookingForm: React.FC<BookingFormProps> = ({
  ...
  const handleSubmitOrder = (e: React.FormEvent) => {
    ...
    onBookingSubmit({
      ...
      date,
      timeSlot,
      technicianId: preselectedTech?.status === 'vacation' ? '' : preselectedTech?.id,
      technicianName: preselectedTech?.status === 'vacation' ? '' : preselectedTech?.name,
      technicianPhone: preselectedTech?.status === 'vacation' ? '' : preselectedTech?.phone,
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-3xl border border-slate-250 shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-6">
          {preselectedTech && (
            preselectedTech.status === 'vacation' ? (
              <div className="mb-4 bg-amber-50 border border-amber-300 rounded-2xl p-3 flex items-center gap-3 text-right">
                <img
                  ...
                  className="w-10 h-10 rounded-full object-cover border-2 border-amber-400 shadow-xs shrink-0"
                />
                <div className="min-w-0 flex-1">
                  <span className="text-[10px] font-bold text-amber-800 block">تکنسین انتخابی در مرخصی موقت است:</span>
                  <span className="text-xs font-black text-slate-800 block truncate">{preselectedTech.name} (سفارش شما برای نزدیک‌ترین تکنسین فعال ارسال خواهد شد)</span>
                </div>
              </div>
            ) : (
