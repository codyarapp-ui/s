...
const persianNormCache = new Map<string, string>();
const normalizePersianArabic = (str: string): string => {
  if (!str) return '';
  let cached = persianNormCache.get(str);
  if (cached === undefined) {
    cached = str
      .toLowerCase()
      ...
      .trim();
    if (persianNormCache.size < 30000) {
      persianNormCache.set(str, cached);
    }
  }
  return cached;
};

const textSearchNormCache = new Map<string, string>();
const normalizeTextForSearch = (str: string): string => {
  if (!str) return '';
  let cached = textSearchNormCache.get(str);
  if (cached === undefined) {
    cached = str
      .toLowerCase()
      .replace(/[\u064A\u06CC]/g, 'ی')
      ...
      .trim();
    if (textSearchNormCache.size < 30000) {
      textSearchNormCache.set(str, cached);
    }
  }
  return cached;
};

const codeNormCache = new Map<string, string>();
const normalizeCode = (code: string): string => {
  if (!code) return '';
  let cached = codeNormCache.get(code);
  if (cached === undefined) {
    cached = code.toLowerCase().trim().replace(/[-_\s.]/g, '');
    if (codeNormCache.size < 30000) {
      codeNormCache.set(code, cached);
    }
  }
  return cached;
};
...
interface PrecompiledBrand {
  english: string;
  persianAliases: Array<{ raw: string; norm: string; searchNorm: string }>;
}

const PRECOMPILED_BRANDS: PrecompiledBrand[] = Object.entries(brandAliases).map(([english, list]) => ({
  english: english.toLowerCase(),
  persianAliases: list.map(p => ({
    raw: p,
    norm: normalizePersianArabic(p),
    searchNorm: normalizeTextForSearch(p),
  })),
}));
...
interface PrecompiledCategory {
  ...
  canonicalSearchNorm: string;
  aliases: Array<{ raw: string; norm: string; searchNorm: string }>;
}

const PRECOMPILED_CATEGORIES: PrecompiledCategory[] = Object.entries(categoryAliases).map(([canonical, list]) => ({
  canonical,
  canonicalNorm: normalizePersianArabic(canonical),
  canonicalSearchNorm: normalizeTextForSearch(canonical),
  aliases: list.map(a => ({
    raw: a,
    norm: normalizePersianArabic(a),
    searchNorm: normalizeTextForSearch(a),
  })),
}));

const matchesToken = (fieldValue: string, token: string): boolean => {
  if (!fieldValue || !token) return false;
  const fLower = fieldValue.toLowerCase().trim();
  const tLower = token.toLowerCase().trim();
  if (!fLower || !tLower) return false;

  // Direct fast contains check
  ...
  // 1b. Check without spaces / hyphens to handle compound words
  ...
  // If token is very short, avoid checking entire alias dictionaries
  if (tLower.length < 2) return false;

  // 2. Brand alias lookup with precompiled structures
  for (let i = 0; i < PRECOMPILED_BRANDS.length; i++) {
    const b = PRECOMPILED_BRANDS[i];
    const fNormMatchesBrand = fNorm.includes(b.english) || b.english.includes(fNorm);
    const tokenMatchesBrand = tNorm.includes(b.english) || b.english.includes(tNorm);

    if (fNormMatchesBrand) {
      for (let j = 0; j < b.persianAliases.length; j++) {
        const p = b.persianAliases[j];
        if (
          tNorm.includes(p.norm) || p.norm.includes(tNorm) ||
          tSearchNorm.includes(p.searchNorm) || p.searchNorm.includes(tSearchNorm)
        ) {
          return true;
        }
      }
    ...
    if (tokenMatchesBrand) {
      for (let j = 0; j < b.persianAliases.length; j++) {
        const p = b.persianAliases[j];
        if (
          fNorm.includes(p.norm) || p.norm.includes(fNorm) ||
          fSearchNorm.includes(p.searchNorm) || p.searchNorm.includes(fSearchNorm)
        ) {
          return true;
        }
      }
  ...
  // 3. Category alias lookup with precompiled structures
  for (let i = 0; i < PRECOMPILED_CATEGORIES.length; i++) {
    const c = PRECOMPILED_CATEGORIES[i];
    const fNormMatchesCanonical =
      fNorm.includes(c.canonicalNorm) || c.canonicalNorm.includes(fNorm) ||
      fSearchNorm.includes(c.canonicalSearchNorm) || c.canonicalSearchNorm.includes(fSearchNorm);

    const tokenMatchesCanonical =
      tNorm.includes(c.canonicalNorm) || c.canonicalNorm.includes(tNorm) ||
      tSearchNorm.includes(c.canonicalSearchNorm) || c.canonicalSearchNorm.includes(tSearchNorm);

    if (fNormMatchesCanonical) {
      for (let j = 0; j < c.aliases.length; j++) {
        const a = c.aliases[j];
        if (
          tNorm.includes(a.norm) || a.norm.includes(tNorm) ||
          tSearchNorm.includes(a.searchNorm) || a.searchNorm.includes(tSearchNorm)
        ) {
          return true;
        }
      }
    ...
    if (tokenMatchesCanonical) {
      for (let j = 0; j < c.aliases.length; j++) {
        const a = c.aliases[j];
        if (
          fNorm.includes(a.norm) || a.norm.includes(fNorm) ||
          fSearchNorm.includes(a.searchNorm) || a.searchNorm.includes(fSearchNorm)
        ) {
          return true;
        }
      }
...
interface ErrorSearchProps {
  ...
  onFilterParts: (category: string, brand: string, errorCode?: string, relatedParts?: string[]) => void;
...
export const ErrorSearch: React.FC<ErrorSearchProps> = ({
  ...
  const isPremium = Boolean(
    ...
    (currentUser?.subscription?.end_date && new Date(currentUser.subscription.end_date) > new Date())
  );
  ...
  React.useEffect(() => {
    if (!onSearchCriteriaChange) return;
  ...
  }, [query, selectedCategory, selectedBrand, selectedModel, searchType, onSearchCriteriaChange]);
  ...
  const suggestions = React.useMemo(() => {
    ...
    for (let i = 0; i < errorCodes.length; i++) {
      const err = errorCodes[i];
      let matchesAll = true;
      for (let j = 0; j < meaningfulTokens.length; j++) {
        const token = meaningfulTokens[j];
        const matched =
  ...
  const availableBrands = React.useMemo(() => {
    ...
    const catNorm = normalizePersianArabic(resolvedCategory);
    const catSearchNorm = normalizeTextForSearch(resolvedCategory);
    
    // Scan all error codes and see what brands are registered under this category
    const brandsFromErrors: string[] = [];
    for (let i = 0; i < errorCodes.length; i++) {
      const err = errorCodes[i];
      if (err.category && (
        normalizePersianArabic(err.category) === catNorm ||
        normalizeTextForSearch(err.category) === catSearchNorm
      )) {
        brandsFromErrors.push(err.brand);
      }
    }
      
    // Scan spare parts as well
    const brandsFromParts: string[] = [];
    for (let i = 0; i < spareParts.length; i++) {
      const part = spareParts[i];
      if (part.category && (
        normalizePersianArabic(part.category) === catNorm ||
        normalizeTextForSearch(part.category) === catSearchNorm
      )) {
        if (Array.isArray(part.compatibility)) {
          for (let j = 0; j < part.compatibility.length; j++) {
            brandsFromParts.push(part.compatibility[j]);
          }
        }
      }
    }
  ...
  const availableModels = React.useMemo(() => {
    ...
    for (let i = 0; i < errorCodes.length; i++) {
      ...
      if (matchCat && matchBrand && err.model) {
        modelsFromErrors.push(err.model);
      }
    }
  ...
  const filteredCodes = React.useMemo(() => {
    ...
    if (cleanQuery && isErrorCodeToken(cleanQuery) && otherFiltersEmpty) {
      const cleanNorm = normalizeCode(cleanQuery);
      return errorCodes.filter(err => normalizeCode(err.code) === cleanNorm);
    }

    const catNorm = resolvedCategory ? normalizePersianArabic(resolvedCategory) : '';
    const catSearchNorm = resolvedCategory ? normalizeTextForSearch(resolvedCategory) : '';
    const brandNorm = resolvedBrand ? normalizePersianArabic(resolvedBrand) : '';
    const brandSearchNorm = resolvedBrand ? normalizeTextForSearch(resolvedBrand) : '';
    const modelNorm = resolvedModel ? normalizePersianArabic(resolvedModel) : '';
    const modelSearchNorm = resolvedModel ? normalizeTextForSearch(resolvedModel) : '';
    ...
    const hasQuery = meaningfulTokens.length > 0;

    return errorCodes.filter(err => {
      if (resolvedCategory) {
        if (!err.category) return false;
        const errCatNorm = normalizePersianArabic(err.category);
        const errCatSearch = normalizeTextForSearch(err.category);
        const matchCat =
          errCatNorm.includes(catNorm) ||
          catNorm.includes(errCatNorm) ||
          errCatSearch.includes(catSearchNorm) ||
          catSearchNorm.includes(errCatSearch);
        if (!matchCat) return false;
      }

      if (resolvedBrand) {
        if (!err.brand) return false;
        const errBrandNorm = normalizePersianArabic(err.brand);
        const errBrandSearch = normalizeTextForSearch(err.brand);
        const matchBrand =
          errBrandNorm.includes(brandNorm) ||
          brandNorm.includes(errBrandNorm) ||
          errBrandSearch.includes(brandSearchNorm) ||
          brandSearchNorm.includes(errBrandSearch);
        if (!matchBrand) return false;
      }

      if (resolvedModel) {
        if (!err.model) return false;
        const errModelNorm = normalizePersianArabic(err.model);
        const errModelSearch = normalizeTextForSearch(err.model);
        const matchModel =
          errModelNorm.includes(modelNorm) ||
          modelNorm.includes(errModelNorm) ||
          errModelSearch.includes(modelSearchNorm) ||
          modelSearchNorm.includes(errModelSearch);
        if (!matchModel) return false;
      }

      if (!hasQuery) {
        return !!(resolvedCategory || resolvedBrand || resolvedModel);
      }
  ...
  }, [errorCodes, deferredQuery, resolvedCategory, resolvedBrand, resolvedModel]);
  ...
  }, [filteredCodes, deferredQuery]);

  // Overall filtered common problems based on query + filters
  const filteredProblems = React.useMemo(() => {
    const cleanQuery = deferredQuery.trim().toLowerCase();
    const catNorm = selectedCategory ? normalizePersianArabic(selectedCategory) : '';
    const catSearchNorm = selectedCategory ? normalizeTextForSearch(selectedCategory) : '';
    const brandNorm = selectedBrand ? normalizePersianArabic(selectedBrand) : '';
    const brandSearchNorm = selectedBrand ? normalizeTextForSearch(selectedBrand) : '';
    ...
    return commonProblems.filter(prob => {
      if (selectedCategory) {
        const pCatNorm = normalizePersianArabic(prob.category);
        const pCatSearch = normalizeTextForSearch(prob.category);
        const matchCat =
          pCatNorm === catNorm ||
          pCatSearch === catSearchNorm ||
          prob.category === 'عمومی';
        if (!matchCat) return false;
      }

      if (selectedBrand) {
        const pBrandNorm = normalizePersianArabic(prob.brand);
        const pBrandSearch = normalizeTextForSearch(prob.brand);
        const matchBrand =
          pBrandNorm.includes(brandNorm) ||
          brandNorm.includes(pBrandNorm) ||
          pBrandSearch.includes(brandSearchNorm) ||
          brandSearchNorm.includes(pBrandSearch) ||
          prob.brand === 'عمومی';
        if (!matchBrand) return false;
      }
  ...
  }, [commonProblems, deferredQuery, selectedCategory, selectedBrand]);
  ...
  }, [realTechDocs, searchType, deferredQuery, resolvedCategory, resolvedBrand, resolvedModel]);
  ...
  const handleSelectProblemAsErrorCode = (prob: CommonProblem) => {
    ...
    const mapped: ErrorCode = {
      ...
      isApproved: true,
      video_url: prob.video_url || prob.videoUrl || (prob as any).video_url || '',
      videoUrl: prob.video_url || prob.videoUrl || (prob as any).video_url || ''
    };
      ...
      <div className="bg-gradient-to-br from-white to-slate-50/40 rounded-3xl border-2 border-blue-600/25 p-6 sm:p-8 shadow-lg shadow-blue-900/5 relative overflow-visible z-50">
        ...
        <div className="mb-6 relative z-10">
          ...
          <h1 className="text-base sm:text-lg font-black text-slate-900 flex items-center gap-2">
            ...
            <span>جستجوی هوشمند و عیب‌یابی کدهای خطای لوازم خانگی و پکیج کدیار۲۴</span>
          </h1>
      ...
      <div className="opacity-100 mt-6 transition-all duration-300 w-full max-w-full overflow-hidden">
        ...
        <div className="space-y-6 w-full max-w-full overflow-hidden">
          ...
          <div className="bg-white -mx-4 md:mx-0 rounded-none md:rounded-3xl border-x-0 md:border border-slate-200/90 shadow-xs overflow-hidden w-auto max-w-none">
                    ...
                    <div className="space-y-6">
                      ...
                      {hasDbContent && (
                        <div className="bg-white border-2 border-slate-200/80 rounded-3xl p-5 sm:p-6 space-y-5 shadow-xs relative">
                          ...
                          {(selectedError.video_url || (selectedError as any).videoUrl) && (
                            <div className="bg-gradient-to-r from-rose-500/10 to-amber-500/10 border border-rose-500/15 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                              ...
                              <button
                                type="button"
                                onClick={() => setActiveVideoUrl(selectedError.video_url || (selectedError as any).videoUrl || null)}
                          ...
                          {/* Dedicated Video Training & Repair Section at End of Steps */}
                          {(selectedError.video_url || (selectedError as any).videoUrl) && (
                            <div className="bg-gradient-to-r from-red-500/10 via-rose-500/10 to-amber-500/10 border border-red-500/20 rounded-2xl p-4 sm:p-5 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xs">
                              <div className="flex items-center gap-3.5 text-right w-full md:w-auto">
                                <div className="w-12 h-12 rounded-2xl bg-red-600 flex items-center justify-center text-white shrink-0 shadow-md">
                                  <Video className="w-6 h-6" />
                                </div>
                                <div className="space-y-1">
                                  <div className="flex items-center gap-2">
                                    <span className="bg-red-100 text-red-700 text-[9.5px] font-black px-2 py-0.5 rounded-md">فیلم آموزشی اختصاصی</span>
                                    <h4 className="text-xs sm:text-sm font-black text-slate-800">مشاهده ویدیو آموزشی و نحوه رفع خطای {selectedError.code}</h4>
                                  </div>
                                  <p className="text-[10.5px] text-slate-500 font-sans leading-relaxed">مشاهده مراحل عملی عیب‌یابی و تعمیر این ارور با پلیر استاندارد</p>
                                </div>
                              </div>
                              <button
                                type="button"
                                onClick={() => setActiveVideoUrl(selectedError.video_url || (selectedError as any).videoUrl || null)}
                                className="bg-red-600 hover:bg-red-700 text-white font-black text-xs px-6 py-3 rounded-xl transition-all cursor-pointer flex items-center gap-2 shadow-md hover:shadow-lg active:scale-95 w-full md:w-auto justify-center shrink-0"
                              >
                                <Play className="w-4 h-4 fill-current" />
                                <span>پخش آنلاین ویدیو آموزشی</span>
                              </button>
                            </div>
                          )}
                        </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 p-4 space-y-3">
                  ...
                  <div className="space-y-2">
                    {(() => {
                      ...
                      const relParts = Array.isArray(selectedError.relatedParts) ? selectedError.relatedParts : [];

                      // 1. Calculate matching Warehouse Spare Parts (spareParts)
                      let matchingWarehouseParts: SparePart[] = [];
                      if (relParts.length > 0) {
                        matchingWarehouseParts = (spareParts || []).filter(p => relParts.includes(p.id) || relParts.includes((p as any).code));
                      } else {
                        const getWarehousePartScore = (part: SparePart) => {
                          let score = 0;
                          const partName = cleanStr(part.name);
                          const partDesc = cleanStr(part.description || '');
                          const errCode = cleanStr(selectedError.code);
                          
                          if (partName.includes(errCode) || cleanStr(part.code) === errCode) score += 50;
                          ...
                          for (const kw of keyWords) {
                            if (errorText.includes(kw) && (partName.includes(kw) || partDesc.includes(kw))) {
                              score += 25;
                            }
                          }
                          return score;
                        ...
                        matchingWarehouseParts = (spareParts || [])
                          ...
                          <div className="space-y-2">
                            <div className="bg-emerald-50 border border-emerald-200/80 p-2 rounded-xl text-[10px] text-emerald-800 font-bold flex items-center justify-between">
                              <span className="flex items-center gap-1.5">
                                ...
                                قطعه یدکی اختصاصی مرتبط با کد خطای {selectedError.code}
                              </span>
                      ...
                      // PRIORITY 2: Fallback to Affiliate Store (affiliateProducts) if Warehouse is Empty
                      let matchingAffiliateProducts: any[] = [];
                      if (relParts.length > 0) {
                        matchingAffiliateProducts = (affiliateProducts || []).filter(ap => relParts.includes(ap.id) || relParts.includes(ap.code));
                      } else {
                        const getAffiliateProductScore = (prod: any) => {
                          let score = 0;
                          const prodName = cleanStr(prod.name || prod.title);
                          const prodDesc = cleanStr(prod.description || '');
                          const errCode = cleanStr(selectedError.code);
                          
                          if (prodName.includes(errCode)) score += 50;

                          const errorText = `${selectedError.code} ${selectedError.title} ${selectedError.description || ''} ${(selectedError.causes || []).join(' ')} ${(selectedError.steps || []).join(' ')}`.toLowerCase();
                          const keyWords = ['برد', 'پمپ', 'شیر', 'سنسور', 'موتور', 'خازن', 'ترموستات', 'دیفراست', 'فن', 'جرقه', 'شیر برقی', 'کلید', 'پرشر', 'ntc', 'مبدل', 'گیج', 'فلومتر', 'فلوسوئیچ', 'گرمایش', 'حرارتی', 'انتیسی', 'المنت', 'دما'];
                          for (const kw of keyWords) {
                            if (errorText.includes(kw) && (prodName.includes(kw) || prodDesc.includes(kw))) {
                              score += 25;
                            }
                          }
                          return score;
                        ...
                        matchingAffiliateProducts = (affiliateProducts || [])
                          ...
                          <div className="space-y-2">
                            <div className="bg-purple-50 border border-purple-200/80 p-2 rounded-xl text-[10px] text-purple-900 font-bold flex items-center justify-between">
                              <span className="flex items-center gap-1.5">
                                ...
                                تامین از فروشگاه همکار برای کد خطای {selectedError.code}
                              </span>
                      ...
                      return (
                        <div className="border border-dashed border-slate-200 bg-slate-50/70 rounded-2xl p-4 text-center space-y-2">
                          ...
                          <p className="text-xs font-bold text-slate-700">قطعه یدکی اختصاصی ثبت‌شده برای کد خطای «{selectedError.code}» موجود نیست</p>
                          <p className="text-[10.5px] text-slate-500 leading-relaxed max-w-sm mx-auto">
                            مدیر سایت هنوز قطعه‌ای برای کد خطای «{selectedError.code}» ({selectedError.brand || ''} {selectedError.model || ''}) در انبار متصل نکرده است.
                          </p>
            ...
            {/* Related Parts specific to this Error Code */}
            {selectedError && (
              <div className="border-t border-slate-150 p-4 sm:p-6 lg:p-8 bg-slate-50/50">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 text-right font-sans">
                  <div>
                    <h3 className="font-extrabold text-xs sm:text-xs text-slate-950 flex items-center gap-1.5 justify-start">
                      <Cpu className="w-4 h-4 text-blue-600" />
                      <span>قطعات یدکی اختصاصی مرتبط با کد خطای «{selectedError.code}»</span>
                    </h3>
                    <p className="text-[10px] text-slate-500 mt-1 leading-normal">
                      لیست قطعات یدکی فنی تعیین‌شده برای رفع خطای {selectedError.code} {selectedError.brand ? `دستگاه ${selectedError.brand}` : ''} با تضمین اصالت و سلامت
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      onFilterParts(selectedError.category, selectedError.brand, selectedError.code, selectedError.relatedParts);
                    }}
                  ...
                  >
                    <span>نمایش در فروشگاه قطعات</span>
                    <ChevronLeft className="w-3.5 h-3.5" />
                  </button>
                </div>
                ...
                {(() => {
                  const relParts = Array.isArray(selectedError.relatedParts) ? selectedError.relatedParts : [];
                  let matchingParts: SparePart[] = [];
                  if (relParts.length > 0) {
                    matchingParts = spareParts.filter(p => relParts.includes(p.id) || relParts.includes((p as any).code));
                  } else {
                    const errCodeLower = (selectedError.code || '').toLowerCase().trim();
                    matchingParts = spareParts.filter(part => {
                      const pName = (part.name || '').toLowerCase();
                      const pDesc = (part.description || '').toLowerCase();
                      return errCodeLower && (pName.includes(errCodeLower) || pDesc.includes(errCodeLower));
                    });
                  }

                  if (matchingParts.length === 0) {
                    return (
                      <div className="text-center py-8 px-4 text-xs text-slate-500 bg-white border border-dashed border-slate-200 rounded-2xl space-y-3">
                        <Cpu className="w-8 h-8 text-slate-300 mx-auto" />
                        <p className="font-bold text-slate-700">برای کد خطای «{selectedError.code}» قطعه اختصاصی در دیتابیس انبار متصل نشده است.</p>
                        <p className="text-[11px] text-slate-400 max-w-md mx-auto">
                          می‌توانید برای تشخیص عیب دقیق و استعلام موجودی با تکنسین‌های فنی تماس حاصل فرمایید یا کلیه قطعات دسته‌بندی «{selectedError.category}» را در فروشگاه بررسی نمایید.
                        </p>
                        <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
                          <button
                            type="button"
                            onClick={() => onFilterParts(selectedError.category, selectedError.brand)}
                            className="text-[10px] text-blue-600 bg-blue-50 hover:bg-blue-100 font-bold px-3 py-2 rounded-xl transition-all cursor-pointer"
                          >
                            مشاهده تمام قطعات {selectedError.category}
                          </button>
                          <button
                            type="button"
                            onClick={() => onBookRepair(selectedError)}
                        ...
                        <div className="space-y-2.5 text-right">
                          ...
                          {!isExpanded ? (
                            ...
                            <div className="space-y-3 pt-2 text-[11px] leading-relaxed border-t border-slate-150 animate-in fade-in duration-200 text-right">
                              ...
                              {/* Video Guide for Common Problem */}
                              {(prob.video_url || prob.videoUrl || (prob as any).video_url) && (
                                <div className="bg-gradient-to-r from-red-500/10 via-rose-500/10 to-amber-500/10 border border-red-500/20 rounded-xl p-3 sm:p-3.5 flex flex-col sm:flex-row items-center justify-between gap-3 text-right">
                                  <div className="flex items-center gap-3 text-right w-full sm:w-auto">
                                    ...
                                    <div className="space-y-0.5">
                                      <div className="flex items-center gap-1.5">
                                        <span className="bg-red-100 text-red-700 text-[8.5px] font-black px-1.5 py-0.5 rounded">ویدیو آموزشی</span>
                                        <h5 className="text-[11px] font-black text-slate-800">فیلم ویدیویی آموزش رفع این مشکل</h5>
                                      </div>
                                      <p className="text-[9.5px] text-slate-500 font-sans">مشاهده فیلم آموزشی و روش گام‌به‌گام رفع عیب با پلیر آنلاین</p>
                                    </div>
                                  </div>
                                  <button
                                    type="button"
                                    onClick={() => setActiveVideoUrl(prob.video_url || prob.videoUrl || (prob as any).video_url || null)}
                                    className="bg-red-600 hover:bg-red-700 text-white font-extrabold text-[10.5px] px-4 py-2 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 shadow-sm active:scale-95 shrink-0 w-full sm:w-auto justify-center"
                                  >
                                    <Play className="w-3.5 h-3.5 fill-current" />
                                    <span>پخش فیلم آموزشی</span>
                                  </button>
                                </div>
                              )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl w-full max-w-3xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-4 sm:p-6 space-y-4">
              {(() => {
                const cleanUrl = (activeVideoUrl || '').trim();
                const isDirect = /\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i.test(cleanUrl) || 
                ...
                let embedUrl = '';
                const aparatMatch = cleanUrl.match(/(?:aparat\.com\/(?:v\/|video\/video\/embed\/videohash\/|embed\/))([a-zA-Z0-9_-]+)/i);
                const youtubeMatch = cleanUrl.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{6,})/i);
                
                if (aparatMatch && aparatMatch[1]) {
                  embedUrl = `https://www.aparat.com/video/video/embed/videohash/${aparatMatch[1]}/vt/frame?recom=none`;
                ...
                if (isDirect) {
                  ...
                  return (
                    <div className="bg-slate-800 border border-slate-700/60 rounded-2xl p-6 text-center space-y-4 max-w-md mx-auto">
                      ...
                      <div className="space-y-1">
                        <h4 className="text-xs font-bold text-slate-200">سیستم پخش خودکار ویدیو آماده است</h4>
                        <p className="text-[10px] text-slate-400">می‌توانید مستقیماً ویدیو آموزشی را از طریق لینک منبع مشاهده فرمایید.</p>
                      </div>
                      <a
                        href={cleanUrl}
                        target="_blank"
